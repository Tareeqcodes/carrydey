...
export default function DriverPortalPage({ params }) {
  ...
  const handleConfirmDelivery = async (enteredOTP) => {
    ...
    try {
      ...
      // Free up driver in driver table
      if (delivery.driverId) {
        try {
          const driverRow = await tablesDB.getRow({
            databaseId: process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID,
            tableId: process.env.NEXT_PUBLIC_APPWRITE_DRIVER_COLLECTION_ID,
            rowId: delivery.driverId,
          });
          const remaining = (driverRow?.assignedDelivery || '')
            .split(',')
            .filter((id) => id && id !== delivery.$id);
          await tablesDB.updateRow({
            databaseId: process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID,
            tableId: process.env.NEXT_PUBLIC_APPWRITE_DRIVER_COLLECTION_ID,
            rowId: delivery.driverId,
          ...
          // non-critical, agency can fix manually
        }
      }
            ...
            <div className="flex gap-3">
              ...
              {delivery?.guestPhone && (
                <a
                  ...
                  className="w-8 h-8 rounded-xl flex items-center justify-center bg-green-50 border border-green-200 flex-shrink-0"
                >
                  <Phone className="w-3.5 h-3.5 text-green-600" />
                </a>
              )}
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                {(delivery?.guestPhone || delivery?.dropoffPhone) && (
                  <a
                    href={`tel:${delivery.guestPhone || delivery.dropoffPhone}`}
