...
import AgencyBooking from '@/components/Agencytrack/AgencyBooking';
...
import { Building2, Loader2, AlertCircle, User, Phone, Sparkles } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { BrandColorsProvider } from '@/hooks/BrandColors';
import { AgencyPricingProvider, parseAgencyPricing } from '@/hooks/Agencypricing';

function checkAvailability(agencyData) {
  if (!agencyData?.operationalHours) return { isOpen: true, message: '' };
  let hours;
  try { hours = JSON.parse(agencyData.operationalHours); } catch { return { isOpen: true, message: '' }; }
  const now = new Date();
  const dayName = now.toLocaleDateString('en-US', { weekday: 'long' });
  const nowMin = now.getHours() * 60 + now.getMinutes();
  const day = hours[dayName];
  if (!day?.open) return { isOpen: false, message: `Closed today (${dayName})` };
  const [fh, fm] = day.from.split(':').map(Number);
  const [th, tm] = day.to.split(':').map(Number);
  if (nowMin < fh * 60 + fm) return { isOpen: false, message: `Opens today at ${day.from}` };
  if (nowMin >= th * 60 + tm) {
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      if (hours[next]?.open) { nextMsg = `${next} at ${hours[next].from}`; break; }
    }
    return { isOpen: false, message: `Closed for today${nextMsg ? `. Opens ${nextMsg}` : ''}` };
  }
...
export default function AgencyBookingPage() {
  ...
  const [brandColors, setBrandColors] = useState({ primary: '#3A0A21', secondary: '#5A1A41', accent: '#8B2E5A' });
  ...
  // ✅ all 5 fields populated at once in handlePackageConfirmed
  const [pendingDelivery, setPendingDelivery] = useState({
    pickup: null, dropoff: null, routeData: null, packageDetails: null, fareDetails: null,
  });

  useEffect(() => { fetchAgencyDetails(); }, [agencyId]);
  ...
  const fetchAgencyDetails = async () => {
    try {
      ...
      if (response.rows.length > 0) {
        ...
        if (agencyData.brandColors) { try { setBrandColors(JSON.parse(agencyData.brandColors)); } catch {} }
  ...
  const handlePackageConfirmed = (packageDetails, fareDetails, pickup, dropoff, routeData) => {
    setPendingDelivery({ pickup, dropoff, routeData, packageDetails, fareDetails });
  ...
  const saveDeliveryToAppwrite = async () => {
    const { pickup, dropoff, routeData, packageDetails, fareDetails } = pendingDelivery;
    ...
    try {
      ...
      await tablesDB.createRow({
        ...
        data: {
          ...
          dropoffPhone: guestInfo.dropoffPhone || packageDetails?.dropoffContact?.dropoffPhone,
          ...
          offeredFare: fareDetails?.offeredFare ? parseInt(fareDetails.offeredFare) : null,
    ...
    <BrandColorsProvider initialColors={brandColors}>
      <AgencyPricingProvider pricing={agencyPricing}>
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
          <Closedagencymodal
            isOpen={showClosedModal} onDismiss={() => setShowClosedModal(false)}
            agencyName={agency.name} message={availability.message}
            brandColors={brandColors} logoUrl={agency.logoUrl}
          />
          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
              <div className="flex items-center gap-4">
                {agency.logoUrl ? (
                  <div className="w-14 h-14 rounded-2xl bg-white border-2 flex items-center justify-center shadow-lg overflow-hidden" style={{ borderColor: brandColors.primary }}>
            ...
            {showGuestForm && (
              ...
              >
                ...
                >
                  ...
                  {/* ✅ Form: name + your phone + dropoff phone (no email) */}
                  <form onSubmit={handleGuestInfoSubmit} className="p-6 sm:p-8 space-y-5">
                    {[
                      {
                        label: 'Your Full Name', key: 'name', type: 'text',
                        icon: User, placeholder: 'John Doe',
                        color: brandColors.primary, required: true,
                      },
                      {
                        label: 'Pickup Contact Number', key: 'phone', type: 'tel',
                        icon: Phone, placeholder: '+234 123 456 7890',
                        color: brandColors.secondary, required: true,
                      },
                      {
                        label: 'Dropoff Contact Number', key: 'dropoffPhone', type: 'tel',
                        icon: Phone, placeholder: '+234 987 654 3210',
                        color: brandColors.accent, required: true,
                        labelSuffix: ' (for recipient contact if different)',
                      },
                    ].map(({ label, key, type, icon: Icon, placeholder, color, required, labelSuffix }) => (
                      <div key={key}>
                        <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">
                          {label}<span className="text-gray-400 text-[9px] normal-case font-normal ml-1">{labelSuffix}</span>
                        </label>
