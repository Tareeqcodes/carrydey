...
const DEFAULT_HOURS = DAYS.reduce(
  (acc, day) => ({ ...acc, [day]: { open: true, from: '08:00', to: '18:00' } }),
  {}
);
...
export default function OperationalSettings({
  ...
  const cardClass =
    'bg-white dark:bg-[#1A1A1A] rounded-3xl border border-gray-200/50 dark:border-white/10 shadow-sm overflow-hidden';
  const inputClass =
    'px-3 py-1.5 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-sm font-medium text-gray-900 dark:text-white focus:outline-none focus:border-green-400';
  const numInputClass =
    'w-full px-4 py-3 bg-gray-50 dark:bg-white/5 border-2 border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:border-amber-400 focus:bg-white dark:focus:bg-white/10 transition-all text-gray-900 dark:text-white font-medium';
  ...
  const SaveButton = () => (
    <motion.button
      ...
      onClick={handleSave}
      disabled={saving}
      className="w-full py-4 px-6 bg-[#00C896] text-white rounded-2xl font-bold text-base hover:shadow-2xl hover:shadow-[#00C896]/30 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
    >
      {saving ? (
        <>
      ...
      >
        <div className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-[11px] md:text-sm font-semibold text-gray-900 dark:text-white">
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${d.open ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                  >
                    <span
                  ...
                  <span
                    className={`w-24 text-sm font-medium flex-shrink-0 ${d.open ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-600'}`}
                  >
                    {day}
                  </span>
                  {d.open ? (
      ...
      >
        <div className="p-8">
          <div className="flex items-center gap-3 mb-4">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-all ${showPricing ? 'bg-gradient-to-br from-teal-500 to-cyan-600 shadow-teal-500/20' : 'bg-gradient-to-br from-gray-400 to-gray-500 shadow-gray-400/20'}`}
            >
              {showPricing ? (
            ...
            <button
              ...
              className={`relative w-12 h-6 rounded-full transition-colors flex-shrink-0 ${showPricing ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
              <span
                className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${showPricing ? 'translate-x-6' : ''}`}
              />
            </button>
          </div>
          <div
            className={`rounded-2xl border p-4 transition-all ${showPricing ? 'border-teal-100 dark:border-teal-900/40 bg-teal-50/40 dark:bg-teal-900/10' : 'border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5'}`}
          >
            ...
            {showPricing ? (
              <p className="text-sm text-teal-700 dark:text-teal-400">
                ✓ Fare section is visible — customers can see and adjust their
                offer
              </p>
            ) : (
        ...
        <>
          ...
          >
            <div className="p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    Distance tiers applied automatically — rates taper for
                    longer trips
                  </p>
                </div>
              </div>
              ...
              {base > 0 && perKm > 0 && (
                <div className="mt-5 p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-900/30">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[5, 10, 20].map((km) => (
                      <div
                        key={km}
                        className="text-center p-2 bg-white dark:bg-white/5 rounded-lg border border-amber-100 dark:border-amber-900/20"
                      >
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-3 gap-4">
                {['small', 'medium', 'large'].map((size) => (
                  <div key={size}>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5 capitalize">
                      {size}
                    </label>
                    <input
                      type="number"
                      ...
                      className="w-full px-3 py-2.5 bg-gray-50 dark:bg-white/5 border-2 border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:border-indigo-400 focus:bg-white dark:focus:bg-white/10 transition-all text-gray-900 dark:text-white font-medium"
                    />
                  </div>
                ))}
              </div>
