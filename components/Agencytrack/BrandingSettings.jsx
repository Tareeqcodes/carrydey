...
import { Building2, Save, Upload, Image as ImageIcon, Palette } from 'lucide-react';
...
const COLOR_PRESETS = [
  ...
  { name: 'Bright Yellow', primary: '#3A3A00', secondary: '#5A5A00', accent: '#FFFF00' },
];

const BrandingSettings = ({ agencyData, formData, setFormData, setAgencyData, onSuccess, onError }) => {
  ...
  const applyColorPreset = (preset) => {
    setFormData((prev) => ({
      ...prev,
      brandColors: { primary: preset.primary, secondary: preset.secondary, accent: preset.accent },
    }));
  };
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, logoUrl: fileUrl.href, uploadingLogo: false }));
      ...
      >
        <div className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900 tracking-tight">Agency Logo</h3>
              <p className="text-xs text-gray-500 mt-0.5">Upload your brand logo</p>
            </div>
          </div>
            ...
            <div className="relative group">
              {formData.logoUrl ? (
                <div className="w-20 h-20 rounded-2xl border-2 border-gray-200 overflow-hidden bg-white shadow-sm">
                  <img src={formData.logoUrl} alt="Agency Logo" className="w-full h-full object-contain p-2" />
                </div>
              ) : (
              ...
              >
                {uploadingLogo ? (
                  ...
                  <>
                    ...
                    <span className="text-sm">{formData.logoUrl ? 'Change Logo' : 'Upload Logo'}</span>
                  </>
                )}
              </motion.button>
              <p className="text-xs text-gray-500 mt-2">PNG, JPG up to 2MB. Recommended: 500x500px</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 tracking-tight">Brand Colors</h3>
              <p className="text-xs text-gray-500 mt-0.5">Customize your brand colors to match your identity</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {COLOR_PRESETS.map((preset) => (
                  ...
                  >
                    ...
                    <p className="text-xs font-medium text-gray-700 text-left">{preset.name}</p>
                  </motion.button>
                ))}
              </div>
            </div>
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-xs">{formData.tagline || 'Professional Logistics'}</p>
                  </div>
                </div>
                <div
                ...
                >
                  <span className="text-white font-semibold text-sm">Book Delivery</span>
                </div>
              </div>
            </div>
