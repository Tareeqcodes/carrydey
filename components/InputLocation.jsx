...
export default function InputLocation({
  ...
  // Keep ref in sync so event handlers can read latest state synchronously
  useEffect(() => { dropoffStatesRef.current = dropoffStates; }, [dropoffStates]);
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (containerRef.current && !containerRef.current.contains(e.target)) {
        ...
        setDropoffStates((prev) => prev.map((d) => ({ ...d, showSugg: false })));
        setActiveField(null);
      }
    };
  ...
  const calculateRoute = async (dropoffLoc) => {
    ...
    try {
      ...
      if (data.routes?.[0]) {
        ...
        onRouteCalculated({ distance: dist, duration: dur, estimatedFare: fare, suggestedFare: fare, polyline: r.geometry.coordinates });
      }
  ...
  const useCurrentLocationForPickup = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { longitude, latitude } }) => {
        try {
          const res = await fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?` +
              new URLSearchParams({ access_token: process.env.NEXT_PUBLIC_MAPBOX_TOKEN, types: 'address,poi,place', limit: '1' })
          );
          const data = await res.json();
          if (data.features?.[0]) {
        ...
        } catch { /* silent */ } finally { setGettingLocation(false); }
      },
      ...
      searchAddress(value, (results) => {
        setDropoffStates((prev) => {
          const updated = prev.map((d, i) =>
            i === idx ? { ...d, suggestions: results, showSugg: results.length > 0 } : d
          );
  ...
  const handleDropoffSelect = (idx, s) => {
    const loc = { ...s, geometry: { type: 'Point', coordinates: s.center }, place_name: s.place_name };
    const patch = { location: loc, address: s.place_name, suggestions: [], showSugg: false };
    const next = dropoffStatesRef.current.map((d, i) => (i === idx ? { ...d, ...patch } : d));
    dropoffStatesRef.current = next;
    setDropoffStates(next);
    onDropoffsChange?.(next.map(toClean));
    setActiveField(null);
  ...
  const addDropoff = () => {
    const id = `d${Date.now()}`;
    const next = [...dropoffStatesRef.current, { ...createDropoff(id), suggestions: [], showSugg: false }];
    dropoffStatesRef.current = next;
    setDropoffStates(next);
    onDropoffsChange?.(next.map(toClean));
  };
  ...
  return (
    <div ref={containerRef} className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4 shadow-sm">
      ...
      <div className="relative">
        <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Pickup</label>
        <div
          className="flex items-center border rounded-xl transition-all overflow-hidden"
          style={{
            borderColor: activeField === 'pickup' ? MAROON : pickup ? `${MAROON}55` : '#e5e7eb',
            boxShadow: activeField === 'pickup' ? `0 0 0 3px ${MAROON}15` : 'none',
          }}
        >
          ...
          <input
            ...
            onFocus={() => { setActiveField('pickup'); if (pickupAddress.length >= 3) setShowPickupSugg(true); }}
          ...
          <div className="flex items-center pr-1 gap-0.5">
            {pickupAddress && (
              <button type="button" onClick={() => { setPickupAddress(''); onLocationSelect('pickup', null); }}
                className="p-2 rounded-lg hover:bg-gray-100" style={{ color: '#9ca3af' }}>
                <X className="w-3.5 h-3.5" />
              </button>
            )}
            <button type="button" onClick={useCurrentLocationForPickup} disabled={gettingLocation}
              className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-40" style={{ color: MAROON }}>
        ...
        {showPickupSugg && pickupSugg.length > 0 && (
          <div className="absolute z-50 w-full mt-1.5 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
            {pickupSugg.map((s, i) => (
              <button key={`${s.id}-${i}`} type="button" onClick={() => handlePickupSelect(s)}
      ...
      <div className="space-y-3">
        {dropoffStates.map((d, idx) => (
          <div key={d.id} className="border border-gray-100 rounded-xl p-3 space-y-2.5" style={{ background: `${ORANGE}05` }}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0" style={{ background: ORANGE }}>
                  {idx + 1}
                </div>
                <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Drop-off {idx + 1}</span>
              </div>
              {dropoffStates.length > 1 && (
                <button type="button" onClick={() => removeDropoff(idx)}
                  className="p-1 rounded-lg hover:bg-red-50 transition-colors" style={{ color: '#9ca3af' }}>
            ...
            <div className="relative">
              <div
                ...
                style={{
                  borderColor: activeField === `dropoff-${idx}` ? ORANGE : d.location ? `${ORANGE}55` : '#e5e7eb',
                  boxShadow: activeField === `dropoff-${idx}` ? `0 0 0 3px ${ORANGE}15` : 'none',
                }}
              >
                <div className="pl-3 pr-2 flex-shrink-0">
                ...
                <input
                  ...
                  value={d.address}
                  onChange={(e) => handleDropoffAddressChange(idx, e.target.value)}
                  onFocus={() => {
                    setActiveField(`dropoff-${idx}`);
                    if (d.address.length >= 3)
                      setDropoffStates((prev) => prev.map((x, i) => i === idx ? { ...x, showSugg: true } : x));
                  }}
                  className="flex-1 py-3 text-sm text-gray-900 placeholder-gray-400 bg-transparent outline-none min-w-0"
                />
                {d.address && (
                  <button type="button"
                    onClick={() => {
                      const next = dropoffStatesRef.current.map((x, i) =>
                        i === idx ? { ...x, address: '', location: null, suggestions: [], showSugg: false } : x
                      );
                      dropoffStatesRef.current = next;
                      setDropoffStates(next);
                      onDropoffsChange?.(next.map(toClean));
                    }}
                    className="p-2 rounded-lg hover:bg-gray-100 mr-1" style={{ color: '#9ca3af' }}>
                    <X className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>
              {d.showSugg && d.suggestions.length > 0 && (
                <div className="absolute z-50 w-full mt-1.5 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
                  {d.suggestions.map((s, si) => (
                    <button key={`${s.id}-${si}`} type="button" onClick={() => handleDropoffSelect(idx, s)}
                      className="w-full flex items-start gap-3 px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-b-0 text-left">
      ...
      {showNextButton && pickup && dropoffStates[0]?.location && (
        <button onClick={onCalculate} disabled={calculatingRoute}
